...
public class CirnonavirusChan {
    ...
    public static void main(String[] args) {
        ...
        api.getChannelById(cirnonavirusChanConfig.getLoadChannelId()).ifPresent(channel -> {
            new MessageBuilder()
                    .append("Cirnonavirus-chan has started in version 9.0.4!", MessageDecoration.BOLD)
                    .append("\n\n")
                    .append("Update notes:", MessageDecoration.BOLD)
                    .append("\n")
                    .append("• Now manages the revised landing zone")
                    .append("\n")
                    .append("• Now manages alarm-pantsu opt-in/out")
                    .append("\n\n")
        ...
        api.addMessageCreateListener(event -> {
            ...
            if (event.getMessage().getAuthor().isRegularUser()) {
                ...
                //gatekeeper function
                if (event.getMessage().getContent().equals("/rules")) {
                    MessageAuthor author = event.getMessage().getAuthor();
                    author.asUser().get().addRole(event.getMessage().getServer().get().getRolesByName("Gay Baka Tejones").get(0));
                    event.getMessage().delete();
                }

                //politics opt-in
                if (event.getMessage().getContent().equals("/wearpantsu")) {
                    MessageAuthor author = event.getMessage().getAuthor();
                    author.asUser().get().addRole(event.getMessage().getServer().get().getRolesByName("Pantsu").get(0));
                    event.getMessage().delete();
                }

                //politics opt-out
                if (event.getMessage().getContent().equals("/nopantsu")) {
                    MessageAuthor author = event.getMessage().getAuthor();
                    author.asUser().get().removeRole(event.getMessage().getServer().get().getRolesByName("Pantsu").get(0));
                    event.getMessage().delete();
                }
        ...
        api.addServerMemberJoinListener(event -> {
            ...
            api.getChannelById(cirnonavirusChanConfig.getLandingChannelId()).ifPresent(channel -> {
                new MessageBuilder()
                        .append("Welcome " + mentionTag + "! Please read the instructions in " + "<#" + cirnonavirusChanConfig.getRulesChannelId() + ">" + " to gain access to the wider server.")
                        .send((TextChannel) channel);
        ...
        api.addUserRoleAddListener (event -> {
            ...
            String mentionTag;
            mentionTag = event.getUser().getMentionTag();
            if (event.getRole().toString().toLowerCase().contains("baka tejones")) {
